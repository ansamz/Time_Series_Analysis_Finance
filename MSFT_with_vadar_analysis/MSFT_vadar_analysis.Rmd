...
MSFT$sentiment_average_score <- as.numeric(as.character(MSFT$sentiment_average_score))
...
ix.na = is.na(MSFT$sentiment_average_score)
problematic_values <- MSFT$sentiment_average_score[ix.na]
MSFT$sentiment_average_score[problematic_values] <- NA
# Replace with 0
MSFT$sentiment_average_score[problematic_values] <- 0
MSFT$Index_numeric <- as.numeric(MSFT$Index - as.Date("2000-01-01"))
MSFT$smoothed_polarity_loess <- with(MSFT, loess(sentiment_average_score ~ Index_numeric, span = 0.2)$fitted)

main_plot <- ggplot(data = MSFT, aes(x = Index)) +
  geom_line(aes(y = MSFT.Adjusted, color = "MSFT.Adjusted"), size = 1) +
  ylab("MSFT Adjusted Price") +
  
  geom_line(aes(y = sentiment_average_score, color = "Sentiment Average Score"), size = 1) +
