...
## Forecasting
We will use the VADAR VAR model for forecasting purposes to see if the inclusion of the reddit posts sentiment in our model has an improving effect on the models accuracy when predicting the price movements of MSFT we do some forecasting for our testing period where we include the reddit posts sentiment data of the testing period.
...
"The Causal Relationship between Social Media Sentiment and Stock Return: Experimental Evidence from an Online Message Forum" by Lin et al. (2022) examines the causal effect of social media sentiment on stock returns using an experimental approach. The study finds that positive sentiment can significantly increase same-day stock prices, but the effect is short-lived.

"Impact Of Social Media On Stock Market- A Case Of Sentiment Analysis Using Vader" by Kumar et al. (2021) applies the Vader sentiment analysis tool to Twitter data to assess its impact on stock market movements. The study finds that positive sentiment tweets are associated with increased stock prices, while negative sentiment tweets are associated with decreased stock prices.
