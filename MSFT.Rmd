...
if (!require("ggpl")) install.packages("ggpl", dependencies=TRUE)
...
However, from a practical point of view, our VAR model may still be of some use when applied for forecasting purposes. To see if the inclusion of the news sentiment in our model has an improving effect on the models accuracy in predicting the price movements of MSFT we do some forecasting for our testing period.
...
```{r echo=FALSE, include=TRUE}
forecast <- predict(MSFT_VAR, n.ahead = length(news_test))
plot(forecast)
...
MSFT_ARMA_pred <- forecast(MSFT_benchmark_ARMA, level=95, h = length(news_test))
plot(MSFT_ARMA_pred)
